...
serve(async (req: Request) => {
  ...
  try {
    ...
    try {
      ...
      console.log("Error checking execute_sql function:", error)
      return new Response(JSON.stringify({ 
        error: 'Database functions not initialized', 
        details: 'Please initialize database functions first' 
      }), {
    ...
    try {
      ...
      const tablesExist = tableData && tableData[0].devices_exist && tableData[0].telemetry_exist && tableData[0].apps_exist
      ...
      if (tablesExist) {
        ...
        return new Response(JSON.stringify({ 
          ...
          tables: {
            devices: tableData[0].devices_exist,
            telemetry_history: tableData[0].telemetry_exist,
            device_apps: tableData[0].apps_exist
          }
        }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          status: 200,
      ...
      try {
        const triggerResult = await supabaseAdmin.rpc('check_and_create_telemetry_trigger')
      ...
      try {
        const realtimeResult = await supabaseAdmin.rpc('enable_realtime_tables')
